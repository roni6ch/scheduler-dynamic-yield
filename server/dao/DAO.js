...
const signUser = async (name,family,email, password) => {
  ...
  const addEvent = async (params) => {
    ...
    let result;
    try {
      result = await EventsModel.create({title,room,backgroundColor,start,end,author});
    } catch (e) {
      throw e.toString();
    }
    if (result.length === 0) {
  ...
  const editEvent = async (params) => {
    ...
    let result;
    try {
      result = await EventsModel.findOneAndUpdate({_id:id}, {$set:{title,room,backgroundColor,start,end,author}}, {new: true});
    } catch (e) {
      throw e.toString();
    }
    if (result.length === 0) {
